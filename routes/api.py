...
def register_routes(app):
    ...
    @app.route("/api/td-watch/schedule")
    def td_watch_schedule():
        """Get Nuggets schedule with triple-double tracking."""
        db = get_db()
        cursor = db.cursor()
        
        try:
            cursor.execute(prepare_query('''
                SELECT date, opponent, is_home, location, played, 
                       triple_double, points, rebounds, assists
            ...
            for row in rows:
                game = {
                    ...
                    'location': row[3],
                    'played': bool(row[4]),
                    'tripleDouble': bool(row[5]) if row[4] else None,
                    'stats': {
                        'points': row[6],
                        'rebounds': row[7],
                        'assists': row[8]
    ...
    @app.route("/api/td-watch/pack-pool")
    def td_watch_pack_pool():
        """Get available prize packs for triple-double raffles."""
        db = get_db()
        cursor = db.cursor()
        
        try:
            cursor.execute(prepare_query('''
                SELECT name, description, series, quantity
                FROM td_prize_packs
                WHERE quantity > 0
                ORDER BY name
            '''))
            ...
            print(f"Error fetching pack pool: {e}")
            return jsonify([])

    @app.route("/api/td-watch/raffle-history")
    def td_watch_raffle_history():
        """Get history of triple-double raffle winners."""
        db = get_db()
        cursor = db.cursor()
        
        try:
            cursor.execute(prepare_query('''
                SELECT game_date, pack_name, winner_address, winner_type
                FROM td_raffle_winners
            ...
            for row in rows:
                entry = {
                    ...
                    'packName': row[1],
                    'winnerAddress': row[2],
                    'winnerType': row[3]  # 'swapfest' or 'fastbreak'
                }
                history.append(entry)
            
            return jsonify(history)
        except Exception as e:
            print(f"Error fetching raffle history: {e}")
            return jsonify([])
